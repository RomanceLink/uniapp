	...
	<view class="page">
		<view class="scroll">
			<view class="card hero">
				<view class="title">Sunmi Face 测试页</view>
				<view class="desc">这个页面只对接当前原生插件 `sunmiFace`，用于测试设备信息、授权、初始化、提取特征和人脸库。</view>
				<view class="guide">
					推荐顺序：1. 申请权限 2. 创建句柄 3. 初始化模型 4. 通过文件激活/通过 AppId 激活 5. 提取图片特征 6. 初始化人脸库 7. 入库/搜索/删除
				</view>
			</view>
			...
			<view class="card">
				...
				<view class="section-tip">初始化模型成功后，再做授权。优先使用“通过文件激活”。</view>
				<view class="field">
			...
			<view class="card">
				...
				<view class="field">
					<view class="label">原生人脸识别</view>
					<view class="section-tip">按百度 SDK 的调用方式，直接通过原生 module 打开悬浮识别层或原生识别界面。</view>
					<view class="button-row">
				...
				<view class="field">
					<view class="label">实时检测状态 faceEventMessage</view>
				...
				<view class="field">
					<view class="label">原生预览解码尺寸</view>
					<view class="button-row">
...
export default {
	...
	onLoad() {
		// #ifdef APP-PLUS
		plus.globalEvent.addEventListener('SunmiFaceEvent', (e) => {
			if (e && e.message) {
				this.faceEventMessage = e.message
			}
		})
		// #endif
	},
	...
	methods: {
		...
		invokeAndStream(title, methodName, payload) {
			return new Promise((resolve, reject) => {
				...
				try {
					...
					// startFaceRecognize 只返回最终结果；实时状态通过 plus.globalEvent 监听
					fn(payload, (res) => {
		...
		runOpenFaceDetect() {
			...
			try {
				const payload = this.buildNativeDetectPayload()
				this.faceEventMessage = '正在打开原生识别界面...'
				sunmiFace.openFaceDetect(payload, (res) => {
					if (res && res.message) this.faceEventMessage = res.message
					this.setResult('openFaceDetect', { request: payload, response: res })
				})
			} catch (e) {
